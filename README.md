...
Requires macOS 13+ and the Xcode Command Line Tools (`xcode-select --install`) and of course Claude Code (duh!).
...
- **Authoritative numbers.** Reads the same endpoint Claude Code's `/usage` view uses, so the percentages are real, not estimated from local logs. That endpoint is undocumented, so it could change.
- **Polls every 60 seconds.** One request a minute and never on demand, so it stays well clear of rate limits.
